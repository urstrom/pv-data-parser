                ...
                for tracker_counter in range (len(pv_system['inverters'])):# range(len(line[j]['dc'])):
                    if pv_system['inverters'][tracker_counter]['is_production'] == 1:
                        try:
                            ...
                            sql_string = str(f"insert into solarlog_5min (system_id, inverter_id, inverter_id_recorded, tracker_id, measurement_time, "
                                  f"tz_offset, yield) values ('{pv_system['id']}', {j+1}, {j+1}, {tracker_counter+1}, "
                ...
                if pv_system['inverters'][inverter_candidate_counter]['is_production'] == 1:
                    for tracker_counter in range(pv_system['inverters'][inverter_candidate_counter]['nr_trackers'] + 1): # +1: accomodate ac value
                        try:
                            ...
                            sql_string += f" ({pv_system['id']}, {inverter_counter + 1}, {inverter_counter + 1}, {tracker_counter}, "
                            sql_string += f"{time_string_insert},{data_point},'{datetime_now}'),"
                            insert_count += 1
                            tracker_counter += 1
                        except Exception as e:
                            traceback.print_exc()
                            print(f"Error: {e} at {pv_system['id']} icc {inverter_candidate_counter} ic {inverter_counter} tc {tracker_counter}", file=sys.stderr)
                    inverter_counter += 1
        if insert_count == 0:
            return
